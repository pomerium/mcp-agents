...
- `packages/haiku-agent/` - Example agent implementation demonstrating the basic structure
...
- `npm run start -w @mcp-agents/haiku-agent` - Run the example agent

### Docker

- `docker build -f Dockerfile.haiku-agent -t haiku-agent .` - Build Docker image for example agent
- `docker run --rm haiku-agent` - Run the example agent in Docker container
...
2. Copy and modify `package.json` from haiku-agent
...
- `Dockerfile.haiku-agent` - Docker configuration for the example agent
